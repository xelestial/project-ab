    ...
    const entries: LogEntry[] = [
      {
        ...
        unitId: "u1",
        actionType: "move",
        positionBefore: { row: 0, col: 0 },
        positionAfter: { row: 1, col: 0 },
      },
    ];
    await store.saveLog("g1", entries);
    const retrieved = await store.getLog("g1");
    expect(retrieved).toHaveLength(1);
    expect(retrieved![0]!.actionType).toBe("move");
    expect(retrieved![0]!.positionAfter).toEqual({ row: 1, col: 0 });
  });

  it("overwrites an existing log on re-save", async () => {
    await store.saveLog("g1", [{ gameId: "g1", timestamp: 1, round: 1, turnIndex: 0, playerId: "p1", unitId: "u1", actionType: "pass" }]);
...
describe("GET /api/v1/replays/:gameId", () => {
  ...
  it("returns 200 with entries when replay exists", async () => {
    const replayStore = new MemoryReplayStore();
    const entries: LogEntry[] = [
      { gameId: "game42", timestamp: 1000, round: 1, turnIndex: 0, playerId: "p1", unitId: "u1", actionType: "move" },
      { gameId: "game42", timestamp: 2000, round: 1, turnIndex: 1, playerId: "p2", unitId: "u2", actionType: "attack" },
    ];
    ...
    const body = res.json<{ gameId: string; entries: LogEntry[]; entryCount: number }>();
    expect(body.gameId).toBe("game42");
    expect(body.entryCount).toBe(2);
    expect(body.entries[0]!.actionType).toBe("move");
    expect(body.entries[1]!.actionType).toBe("attack");
  });
});
...
describe("ReplayAdapter", () => {
  ...
  it("replays move actions in order", async () => {
    const entries: LogEntry[] = [
      {
        gameId: "g1", timestamp: 1, round: 1, turnIndex: 0,
        playerId: "p1", unitId: "u1", actionType: "move",
        positionBefore: { row: 0, col: 0 }, positionAfter: { row: 1, col: 0 },
      },
      {
        gameId: "g1", timestamp: 2, round: 1, turnIndex: 1,
        playerId: "p1", unitId: "u1", actionType: "attack",
        positionAfter: { row: 2, col: 0 },
      },
    ];
  ...
  it("skips entries not belonging to this player", async () => {
    const entries: LogEntry[] = [
      { gameId: "g1", timestamp: 1, round: 1, turnIndex: 0, playerId: "p2", unitId: "u2", actionType: "move", positionAfter: { row: 5, col: 5 } },
      { gameId: "g1", timestamp: 2, round: 1, turnIndex: 1, playerId: "p1", unitId: "u1", actionType: "pass" },
    ];
  ...
  it("returns default unit order", async () => {
    const adapter = new ReplayAdapter("p1", []);
